...
$Ctx = New-AzureStorageContext -StorageAccountName $StorageAccountName -StorageAccountKey $LucyStagingStorageUSKey
...
$StorageAccountName = "lucystagingstorageus" 
$Context = New-AzureStorageContext -StorageAccountName $StorageAccountName -StorageAccountKey $LucyStagingStorageUSKey
...
function CompareJsonResult
{
    ...
    if ($CompareJson.Length -eq 0)   
        ...
        $ExpectResult | Out-File  $PSScriptRoot\expectresult.json -Force
        ...
        $ActualResult | Out-File  $PSScriptRoot\actualresult.json -Force

        $context = New-AzureStorageContext -StorageAccountName lucystagingstorageus -StorageAccountKey $LucyStagingStorageUSKey 
        Set-AzureStorageBlobContent  -Container integration-test-result -File $PSScriptRoot\expectresult.json -Context $context -Blob compareresult/$BuildNumber/expectresult.json -Force
        Set-AzureStorageBlobContent  -Container integration-test-result -File $PSScriptRoot\actualresult.json -Context $context -Blob compareresult/$BuildNumber/actualresult.json -Force
        #$DebugRUL = "https://simondu123.github.io/?left=https://lucystagingstorageus.blob.core.windows.net/integration-test-result/$diffexpectresult.json&right=https://lucystagingstorageus.blob.core.windows.net/integration-test-result/$diffactualresult.json"
        $DebugRUL = "http://lucystagingstorageus.blob.core.windows.net/integration-test-result/index.html?build=$BuildNumber"
...
function RemoveJsonFiles()
{
    ...
    Write-host "Removed temp JSON files"
}